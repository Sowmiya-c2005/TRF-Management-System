...
import { getAllTRFs, searchTRF } from "../services/trfService";
...
function buildMonthly(trfs) {
  ...
  trfs.forEach((t) => {
    ...
    if (counts[m]) {
      counts[m].trfs += 1;
      counts[m].docs += 3; // Est documents factor
    }
  });
...
const fadeUp  = { initial: { opacity: 0, y: 15 }, animate: { opacity: 1, y: 0, transition: { duration: 0.4, ease: "easeOut" } } };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    getAllTRFs()
      .then(async (trfsRes) => {
        const trfsList = trfsRes.data || [];
        const details = await Promise.all(
          trfsList.map(async (t) => {
            try {
              const res = await searchTRF(t.trf_number);
              return res.data;
            } catch (err) {
              return null;
            }
        ...
        validDetails.forEach((t) => {
          if (t.project_name) uniqueProjects.add(t.project_name.trim());
          if (t.folders) {
            t.folders.forEach((f) => {
              if (f.files) {
                totalDocs += f.files.length;
        ...
        if (totalBytes >= 1073741824) {
          storageString = `${(totalBytes / 1073741824).toFixed(1)} GB`;
        } else if (totalBytes >= 1048576) {
          storageString = `${(totalBytes / 1048576).toFixed(1)} MB`;
        } else if (totalBytes >= 1024) {
          storageString = `${(totalBytes / 1024).toFixed(1)} KB`;
        } else {
          storageString = `${totalBytes} B`;
        }

        setStats({
          total_trfs: trfsList.length,
          total_documents: totalDocs,
          active_projects: uniqueProjects.size || trfsList.length,
          storage_used: storageString,
        });

        setMonthly(buildMonthly(trfsList));
      })
      .catch((err) => {
        console.error(err);
        setStats({
  ...
  const statCards = [
    ...
    { title: "Total Documents",  value: stats?.total_documents ?? 0,
      subtitle: "Total uploaded files",   icon: <DescriptionRoundedIcon />,
      gradient: "linear-gradient(135deg,#06b6d4,#22d3ee)", glowColor: "#06b6d4", delay: 0.05 },
    { title: "Active Projects",  value: stats?.active_projects ?? 0,
      subtitle: "Unique active names",  icon: <WorkRoundedIcon />,
      gradient: "linear-gradient(135deg,#10b981,#34d399)", glowColor: "#10b981", delay: 0.1 },
    { title: "Storage Used",     value: stats?.storage_used ?? "0 KB",
      subtitle: "Physical storage size",  icon: <StorageRoundedIcon />,
      gradient: "linear-gradient(135deg,#f59e0b,#fbbf24)", glowColor: "#f59e0b", delay: 0.15 },
  ];
  ...
  const QUICK_ACTIONS = [
    ...
    { label: "Export Reports", desc: "Generate spreadsheet", icon: <AssessmentRoundedIcon />, color: "#10b981", bg: "rgba(16,185,129,0.08)", path: "/reports" },
    { label: "Portal Settings", desc: "App preferences", icon: <SettingsRoundedIcon />, color: "#a855f7", bg: "rgba(168,85,247,0.08)", path: "/settings" }
  ];
  ...
  return (
    <motion.div variants={stagger} initial="initial" animate="animate">
        ...
        <Grid container spacing={2.5} sx={{ mb: 4 }} alignItems="stretch">
          <Grid item xs={12} md={8}>
            <Box sx={{
              ...
              <Typography variant="body2" sx={{ color: theme.palette.text.secondary, maxWidth: 550, lineHeight: 1.5 }}>
                You are currently logged in as an <b>{user?.role || "Administrator"}</b>. Access the file system, manage technical requests, and track real-time audit logs below.
              </Typography>
              <Box sx={{ mt: 2.5, display: "flex", gap: 1.5 }}>
                ...
                <Button
                  variant="outlined" startIcon={<FolderOpenRoundedIcon />}
          ...
          <Grid item xs={12} md={4}>
            <Box sx={{
              ...
              <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
                ...
                <Chip icon={<GrainRoundedIcon sx={{ color: "#10b981 !important" }} />} label="Online" size="small" color="success" variant="outlined" sx={{ fontWeight: 700 }} />
              </Box>
      ...
      <motion.div variants={fadeUp}>
        <Typography variant="subtitle2" fontWeight={800} color="text.secondary" sx={{ letterSpacing: "0.1em", mb: 2 }}>QUICK SHORTCUTS</Typography>
            ...
            <Grid item xs={12} sm={6} lg={3} key={i}>
              <Card
                ...
                sx={{
                  ...
                  "&:hover": { borderColor: action.color, boxShadow: `0 12px 30px ${action.color}15` },
              ...
              >
                <CardContent sx={{ p: 2.5, "&:last-child": { pb: 2.5 }, display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  <Box sx={{ minWidth: 0 }}>
                    <Typography variant="subtitle2" fontWeight={700} sx={{ fontSize: "0.85rem", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {action.label}
                    </Typography>
      ...
      <Grid container spacing={2.5} sx={{ mb: 4.5 }}>
            ...
            <Box sx={{ p: 3, borderRadius: "24px", background: cardBg, backdropFilter: "blur(20px)", border: `1px solid ${border}` }}>
              <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 3 }}>
                <Box>
                  ...
                  <Typography variant="caption" sx={{ color: theme.palette.text.secondary }}>
                    Monthly aggregate TRFs and estimated document logs
                  </Typography>
                </Box>
                <Chip icon={<TrendingUpRoundedIcon sx={{ fontSize: "14px !important" }} />} label="Live" color="success" size="small" sx={{ fontWeight: 700 }} />
              </Box>
              <ResponsiveContainer width="100%" height={240}>
                <AreaChart data={monthly} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <ChartTooltip contentStyle={tooltip} />
                  <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: "0.78rem" }} />
                  <Area type="monotone" dataKey="trfs" name="TRFs Created" stroke="#6366f1" strokeWidth={3} fill="url(#dTRF)"  dot={false} />
                  <Area type="monotone" dataKey="docs" name="Files Stored" stroke="#06b6d4" strokeWidth={3} fill="url(#dDocs)" dot={false} />
                </AreaChart>
              </ResponsiveContainer>
            </Box>
          </motion.div>
        ...
        <Grid item xs={12} lg={4}>
          <motion.div variants={fadeUp} style={{ height: "100%" }}>
            <Box sx={{ p: 3, borderRadius: "24px", background: cardBg, backdropFilter: "blur(20px)", border: `1px solid ${border}`, height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              <Box>
                <Typography variant="h6" fontWeight={700} sx={{ fontSize: "1rem", mb: 0.5 }}>Document Breakdown</Typography>
                <Typography variant="caption" sx={{ color: theme.palette.text.secondary }}>File system distribution by category</Typography>
              </Box>
                ...
                {PIE_DATA.map((d) => (
                  <Box key={d.name} sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      ...
                      <Typography variant="caption" sx={{ color: theme.palette.text.secondary, fontSize: "0.75rem", fontWeight: 500 }}>{d.name}</Typography>
                    </Box>
      ...
      <Grid container spacing={2.5}>
        <Grid item xs={12} md={6}>
          <motion.div variants={fadeUp}>
            <Box sx={{ p: 3, borderRadius: "24px", background: cardBg, backdropFilter: "blur(20px)", border: `1px solid ${border}`, minHeight: 330 }}>
              <Typography variant="h6" fontWeight={700} sx={{ fontSize: "1rem", mb: 0.5 }}>Project Tracking</Typography>
              <Typography variant="caption" sx={{ color: theme.palette.text.secondary, display: "block", mb: 3.5 }}>Current status of active pipeline installations</Typography>
                ...
                {PROJECTS.map((p, i) => (
                  <Box key={p.name}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.8 }}>
                      ...
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        <Chip label={p.status} size="small" sx={{ height: 18, fontSize: "0.58rem", fontWeight: 800, background: `${p.color}15`, color: p.color, border: `1px solid ${p.color}30` }} />
                        <Typography variant="caption" sx={{ fontWeight: 800, color: p.color, fontSize: "0.78rem" }}>{p.progress}%</Typography>
                      </Box>
                    </Box>
            ...
            <Box sx={{ p: 3, borderRadius: "24px", background: cardBg, backdropFilter: "blur(20px)", border: `1px solid ${border}`, minHeight: 330 }}>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2 }}>
                ...
                {user?.role === "Admin" ? (
                  ...
                  <Chip label="User Activity" size="small" variant="outlined" sx={{ ml: "auto", fontSize: "0.62rem", fontWeight: 700 }} />
                )}
              </Box>
                ...
                {activities.slice(0, 5).map((act, i) => (
                  <Box key={act.id} sx={{ display: "flex", gap: 2, py: 1.2, position: "relative" }}>
                    ...
                    <Box sx={{ minWidth: 0, flex: 1 }}>
                      <Typography variant="caption" sx={{ fontWeight: 700, fontSize: "0.8rem", display: "block", lineHeight: 1.3, color: "text.primary" }}>
                        {act.action}
                      </Typography>
