  ...
  return (
    <Box>
        ...
        <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", mb: 4, flexWrap: "wrap", gap: 2 }}>
          ...
          <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap" }}>
            <Button variant="outlined" size="small" startIcon={<FileDownloadRoundedIcon />} onClick={exportCSV}
              disabled={loading || !filtered.length} sx={{ borderRadius: "10px", height: 38 }}>Export</Button>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}>
        <Box sx={{ p: 2.5, borderRadius: "18px", background: cardBg, border: `1px solid ${border}`, backdropFilter: "blur(20px)", mb: 2.5, display: "flex", alignItems: "center", gap: 2 }}>
          <TextField fullWidth placeholder="Search by TRF number or project name…" value={search} onChange={e => setSearch(e.target.value)} size="small"
            InputProps={{ startAdornment: <InputAdornment position="start"><SearchRoundedIcon sx={{ fontSize: 18, color: theme.palette.text.secondary }} /></InputAdornment>, sx: { borderRadius: "10px" } }} />
          {search && <Chip label={`${filtered.length} results`} size="small" sx={{ fontSize: "0.72rem", background: "rgba(99,102,241,0.12)", color: "#818cf8" }} />}
        </Box>
      </motion.div>
        ...
        <Box sx={{ borderRadius: "18px", background: cardBg, border: `1px solid ${border}`, backdropFilter: "blur(20px)", overflow: "hidden" }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", px: 3, py: 1.75, background: isDark ? "rgba(99,102,241,0.08)" : "rgba(99,102,241,0.04)", borderBottom: `1px solid ${border}`, gap: 2 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, width: 160, flexShrink: 0 }}>
              ...
              <Typography sx={{ fontSize: "0.72rem", fontWeight: 700, color: theme.palette.text.disabled, letterSpacing: "0.08em", textTransform: "uppercase" }}>TRF Number</Typography>
            </Box>
            <Typography sx={{ fontSize: "0.72rem", fontWeight: 700, color: theme.palette.text.disabled, letterSpacing: "0.08em", textTransform: "uppercase", flex: 1 }}>Project Name</Typography>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, width: 130, flexShrink: 0 }}>
              ...
              <Typography sx={{ fontSize: "0.72rem", fontWeight: 700, color: theme.palette.text.disabled, letterSpacing: "0.08em", textTransform: "uppercase" }}>Created</Typography>
            </Box>
          ...
          {!loading && filtered.length === 0 && (
            <Box sx={{ py: 10, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
              ...
              <Typography sx={{ color: theme.palette.text.secondary, fontWeight: 600 }}>{search ? "No matching TRFs" : "No TRFs yet"}</Typography>
              {!search && can("create_trf") && (
                <Button variant="contained" size="small" startIcon={<AddRoundedIcon />} onClick={() => navigate("/create")}
          ...
          {!loading && filtered.map((trf, i) => (
            <motion.div key={trf.trf_number} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}>
              <Box sx={{
