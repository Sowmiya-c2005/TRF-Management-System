/**
 * ManagerDashboard.jsx — Enterprise Manager Dashboard
...
export default function ManagerDashboard() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([dashRes, trfRes]) => {
        ...
        // Activities — safely map with correct field names
      ...
      .catch(err => {
        console.error("ManagerDashboard loading error:", err);
  ...
  // Auto-refresh every 60 seconds to pick up newly assigned projects
  useEffect(() => {
  ...
  const quickActions = [
    { label:"Search TRF",    icon:<SearchRoundedIcon/>,     color:"#6366f1", path:"/search"    },
    ...
    { label:"All TRFs",      icon:<WorkRoundedIcon/>,       color:"#a855f7", path:"/all"       },
            ...
            <Box sx={{
              ...
              <Box sx={{ position:"relative" }}>
                ...
                <Box sx={{
                  ...
                  border:"1px solid rgba(16,185,129,0.45)", borderRadius:"24px", px:2, py:.6, mb:2.5,
                }}>
                  <Box sx={{ width:8, height:8, borderRadius:"50%", background:"linear-gradient(135deg,#34d399,#22d3ee)", flexShrink:0 }}/>
                  <Typography sx={{ color:"#34d399", fontWeight:800, fontSize:"0.78rem", letterSpacing:1.2, textTransform:"uppercase" }}>
                    Manager Dashboard
                  </Typography>
                </Box>
                <Typography sx={{ color:"#64748b", fontSize:"0.88rem", lineHeight:1.6, maxWidth:460 }}>
                  Overview of assigned projects, document tracking, analytics, and operational metrics.
                </Typography>
              </Box>
            </Box>
          </motion.div>
            ...
            <GlassCard isDark={isDark} sx={{p:3,height:"100%",display:"flex",flexDirection:"column",gap:2}}>
              ...
              <Box sx={{display:"flex",gap:1,flexWrap:"wrap"}}>
                <Chip label={`${assignedTRFs.length} TRFs Assigned`} size="small"
                  sx={{fontSize:"0.7rem",fontWeight:700,background:"rgba(99,102,241,0.12)",color:"#818cf8"}}/>
                {stats?.team_progress !== undefined && (
                  <Chip label={`${stats.team_progress}% Progress`} size="small"
        ...
        <GlassCard isDark={isDark} sx={{p:3, mb:3.5}}>
          <Box sx={{display:"flex",justifyContent:"space-between",alignItems:"center",mb:2.5}}>
            ...
            <Chip
              ...
              size="small"
              sx={{fontWeight:700,fontSize:"0.7rem",background:"rgba(99,102,241,0.12)",color:"#818cf8"}}
            />
          </Box>
            ...
            <GlassCard isDark={isDark} sx={{p:3}}>
              ...
              <ResponsiveContainer width="100%" height={230}>
                <AreaChart data={monthly} margin={{top:0,right:0,left:-20,bottom:0}}>
                  ...
                  <ChartTooltip />
                  <Area type="monotone" dataKey="trfs" name="TRFs Created" stroke="#06b6d4" strokeWidth={2.5} fillOpacity={0.1} fill="#06b6d4" />
                </AreaChart>
              </ResponsiveContainer>
            </GlassCard>
          </motion.div>
        ...
        <Grid item xs={12} lg={4}>
          <motion.div variants={up} style={{height:"100%"}}>
            <GlassCard isDark={isDark} sx={{p:3,height:"100%"}}>
              <Typography sx={{fontWeight:700,fontSize:"0.95rem",color:headingColor,mb:2}}>Recent Operations Activity</Typography>
